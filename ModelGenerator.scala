...
val indexers = Array("NAME_CONTRACT_TYPE","CODE_GENDER","FLAG_OWN_CAR","FLAG_OWN_REALTY","NAME_INCOME_TYPE","NAME_EDUCATION_TYPE","ORGANIZATION_TYPE").map(c => new StringIndexer().setInputCol(c).setOutputCol(c + "_Index"))
...
val encoder = Array("NAME_INCOME_TYPE","NAME_EDUCATION_TYPE","ORGANIZATION_TYPE").map(column => new OneHotEncoder().setInputCol(column+"_Index").setOutputCol(column + "_Vec"))
